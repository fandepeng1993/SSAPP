...
//周工作计划
.controller('wPlanCtrl', function($scope, $state, $filter, $ionicModal, $ionicSlideBoxDelegate, $ionicActionSheet, ConfigService, localStorageService, wPlanService, alertService) {
  //图片头部动画
  ...
  $scope.addPlan = function() {
    //填充图片字段
    $scope.plan.photoUrls = "";
    for(var i=0; i<$scope.reportPhotos.length; i++) {
      $scope.plan.photoUrls = $scope.plan.photoUrls + $scope.reportPhotos[i] + "|";
    }
    if($scope.plan.photoUrls != "") {
      $scope.plan.photoUrls = $scope.plan.photoUrls.substring(0, $scope.plan.photoUrls.length - 1);
    }
    //填充其他字段
    $scope.plan.fromdate = $("#startDate").val();
    $scope.plan.enddate = $("#endDate").val();
    wPlanService.postPlan($scope.plan).then(function (data) {
      if (data.status === 1) {
        //alert("周工作计划发布成功!");
        //$state.go('tab.dash');
        alertService.showAlert($scope, "周工作计划发布成功!", true, "success", 'tab.dash');
      } else {
