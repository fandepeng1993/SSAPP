...
//部门周报
.controller('dwReportCtrl', function($scope, $state, $filter, $ionicModal, $ionicSlideBoxDelegate, $ionicActionSheet, ConfigService, localStorageService, dwReportService, alertService, commonService) {
  ...
  //$scope.$on($state.current.name + "-init", function() {
    ...
    //该部门责任人Id
    var respPeople = localStorageService.get('User').respPeople;
    var userId = localStorageService.get('User').userId;
    var deptId = 0;
    if(respPeople === userId) {
    $scope.deptname = localStorageService.get('User').deptName;
    deptId = localStorageService.get('User').deptId;
    }
    $scope.report = {
      'user': { 'userId': userId },
      'dept': { 'deptId': deptId }
    ...
    $scope.addReport = function() {
      ...
      if(deptId === 0) {
        //alert("您不是部门负责人，无法添加部门周报！");
        alertService.showAlert($scope, '您不是部门负责人，无法添加部门周报！', true, "fail", null);
        return;
      }
      dwReportService.postReport($scope.report).then(function (data) {
        if (data.status === 1) {
          //alert("部门周报发布成功!");
          //$state.go('tab.dash');
          alertService.showAlert($scope, "部门周报发布成功!", true, "success", 'tab.dash');
        } else {
