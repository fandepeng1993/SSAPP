...
//打包查询
.controller('chkpCtrl', function($scope, $state, $window, $timeout, commonService, localStorageService, chkpService, $ionicSideMenuDelegate) {
  ...
  $scope.$on($state.current.name + "-init", function() {
	  ...
	  $scope.expandDetail = function(index, message) {
		...
		var detailHTML = "<tr class=\"detail\"><td colspan=\"6\"><div class=\"content-p\" style=\"line-height:20px;\">" + 
						  "<span style=\"font-weight:bold!important\">重量：</span>" + message.dWeight  +  "kg" +" 　" +
						  "<span style=\"font-weight:bold!important\">支数：</span>" + message.dQuantity  + "支" + "　 "+
						  "<span style=\"font-weight:bold!important\">米数：</span>" + message.dNumbers + "米" + "<br />" +
						  "<span style=\"font-weight:bold!important\">开始时间：</span>" + message.dStarttime   + "<br />" +
						  "<span style=\"font-weight:bold!important\">结束时间：</span>" + message.dEndtime  + 
						  "</div></td></tr>";
	  ...
	  $scope.getData = function () {
		if(curData != null) {
		  ...
		  chkpService.getChkpsByPageNoAndSize(vm.pagination.currentPage, vm.pagination.perPage).then(function (data) {
	  ...
	  $scope.searchManager = function(isInit) {
		$scope.dBarcode = $("#dBarcode").val();
		$scope.contractno = $("#contractno").val();
		$scope.dBoxno = $("#dBoxno").val();
		$scope.totalWeight = 0;
		$scope.totalQuantity = 0;
		curData = [];
		$.each(allData, function(index, item) {
		  //判断工序、框号、合同号和超期都为所选或所填项时，往curData里添加数据。
		  if(item.dBarcode.indexOf($scope.dBarcode) != -1 && item.contractno.indexOf($scope.contractno) != -1 && item.dBoxno.indexOf($scope.dBoxno) != -1) {
			curData.push(item);
		...
		//为统计信息显示而用。
		if($scope.dBarcode === "") {$scope.dBarcode = "所有";}
		if($scope.contractno === "") {$scope.contractno = "所有";}
		if($scope.dBoxno === "") {$scope.dBoxno = "所有";}
...
//打包查询-统计表
.controller('DbcxCtrl', function($scope, $state, commonService, $ionicLoading, chkpService) {
  //$("#loading").show();
  $(".cpcx").hide();
  commonService.showLoading();
  $scope.$on($state.current.name + "-init", function() {
  //$(function () {
    var totalWeight;
    var totalQuantity;
    chkpService.totalWeiAndQua().then(function (data){
